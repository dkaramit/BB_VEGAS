...
LD VEGAS<LD,NDim,NBin,Estimator,RandEn>::Random(LD min, LD max){
    std::uniform_real_distribution<LD> UnDist(min,max);
    return UnDist(RndE) ;
}

template<class LD, int NDim, int NBin, BatchEstimator Estimator, class RandEn>
int VEGAS<LD,NDim,NBin,Estimator,RandEn>::RandomBin(){
...
template<class LD, int NDim, int NBin, BatchEstimator Estimator, class RandEn>
int VEGAS<LD,NDim,NBin,Estimator,RandEn>::RandomBin(int NB){
